...
func TestLimitListener(t *testing.T) {
	const max = 5
	l, err :=net.Listen("tcp", Handle.InfoAddr)
	if err != nil {
		log.Fatalf("warn %v\n", err)
	}
	defer l.Close()
	l = Handle.LimitListener(l, max)

	var open int32
	go http.Serve(l, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if n := atomic.AddInt32(&open, 1); n > max {
			log.Fatalf("%d open connections, want <= %d", n, max)
		}
	...
	for i := 0; i < max; i++ {
		...
		go func() {
			...
			r, err := c.Get("http://" + l.Addr().String()+ "/?path=doc")
			if err != nil {
...
func TestLimitListenerError(t *testing.T) {
	...
	go func() {
		defer close(errCh)
		const n = 2
		ll := Handle.LimitListener(errorListener{}, n)
		for i := 0; i < n+1; i++ {
			_, err := ll.Accept()
			if err != errFake {
